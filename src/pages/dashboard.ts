...
export const dashboardPage = (isPublicThreatIndex = false) =>
  htmlShell({
    title: isPublicThreatIndex
      ? 'Public Threat Index — RealityPulse'
      : 'Command Center — RealityPulse',
    bodyHTML: `
<!-- TOP NAV -->
...
// =====================================================================
// COMMAND CENTER TAB (the hero screen)
// =====================================================================
...
// =====================================================================
// PULSE WHEEL — the hero visual. Three concentric rings.
// =====================================================================
function pulseWheelSVG() {
  // 24h clock; angle 0° = top (midnight UTC), clockwise.
  // Pre-baked events with hour positions matching DEMO_BRIEFING.
  const events = [
  ...
  // Ring radii (SVG coords on 400×400)
  ...
  const ticks = events
    .map((e) => {
      const angle = (e.hour / 24) * 360 - 90 // -90 to start at top
      ...
      return `
        <g class="wheel-tick cursor-pointer" data-title="${e.title}" data-sev="${e.sev}" data-url="${e.url}" data-color="${e.color}">
...
// Threat meter (180° gauge with animated needle)
function threatMeterSVG(value: number) {
  const angle = -90 + (value / 100) * 180 // -90° at 0, +90° at 100
  return `
  <div class="relative">
    <svg viewBox="0 0 240 140" class="w-full">
      ...
      ${[0, 25, 50, 75, 100]
        .map((v) => {
          ...
          return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="#3a4055" stroke-width="1.5" /><text x="${120 + Math.cos(a) * 112}" y="${120 + Math.sin(a) * 112 + 4}" fill="#3a4055" font-family="JetBrains Mono" font-size="10" text-anchor="middle">${v}</text>`
        })
        .join('')}
      <!-- needle -->
      <g style="transform-origin:120px 120px; transform: rotate(${angle - 90}deg);" class="needle" data-needle-angle="${angle - 90}">
    ...
    <div class="text-center mt-2">
      <div class="mono text-3xl font-semibold text-white">${value}<span class="text-base text-gray-500">/100</span></div>
      <div class="text-[10px] mono uppercase text-gray-500 tracking-widest">Elevated</div>
    </div>
  </div>`
}

// Sankey: 12 threats → 3 actions
function sankeySVG() {
  return `
  <svg viewBox="0 0 320 200" class="w-full">
    ...
    ${[
      ...
      .map(
        (n) =>
          `<rect x="10" y="${n.y - 18}" width="14" height="36" fill="${n.color}" rx="2" />
           <text x="32" y="${n.y - 2}" fill="#e7eaf3" font-family="Inter" font-size="11" font-weight="500">${n.label}</text>
           <text x="32" y="${n.y + 12}" fill="${n.color}" font-family="JetBrains Mono" font-size="10">${n.count} threats</text>`,
      )
      .join('')}
    <!-- Target nodes (right) -->
    ${[
      ...
      .map(
        (n) =>
          `<rect x="296" y="${n.y - 14}" width="14" height="28" fill="${n.color}" rx="2" />
           <text x="290" y="${n.y - 18}" fill="#e7eaf3" font-family="Inter" font-size="10" text-anchor="end">${n.label}</text>`,
      )
      .join('')}
    <!-- Flows -->
    ${[
      ...
      .map(
        ([c, sy, ty, w]) =>
          `<path d="M 24 ${sy} C 150 ${sy}, 170 ${ty}, 296 ${ty}" fill="none" stroke="${c}" stroke-opacity="0.35" stroke-width="${w}" />`,
      )
      .join('')}
  </svg>`
}
...
function worldMapPaths() {
  // Very stylized blob world map (visual only).
  return `
...
<section data-pane="competitor" class="tab-pane hidden">
  <div class="grid grid-cols-12 gap-5">
    ...
    <div class="col-span-12 lg:col-span-8 card p-5">
      <div class="flex items-center justify-between mb-3">
        <h3 class="font-semibold">Pricing Diff · stripe.com/pricing</h3>
